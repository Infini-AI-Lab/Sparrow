...
def parallel_nsa_kernel_topk(
    ...
    if IS_VARLEN:
        i_n, i_t = tl.load(q_chunk_indices + i_t * 2).to(tl.int32), tl.load(q_chunk_indices + i_t * 2 + 1).to(tl.int32)
    ...
    q_ptr = q + (bos + i_t) * HQ * D + i_h * G * D

    offs_b = tl.arange(0, BQ) 
    offs_h = tl.arange(0, G)      # [G]
    offs_k = tl.arange(0, D)       # [D]

    offs_row = (offs_b * HQ)[:,None] + offs_h[None,:]
    

    p_q = q_ptr + offs_row[:,:, None] * D + offs_k[None, None, :]  # [BQ, G, D]

    
    mask = ((bos + i_t + offs_b) < eos)[:,None,None]

    b_q = tl.load(p_q, mask=mask, other=0.0)  # [BQ, G, D]
    b_q = tl.reshape(b_q, [BQ * G, D])
    ...
    ################################
    # [BC]
    b_i = tl.full([BQ, BC], NEG_INF, dtype=tl.float32) #[BQ, BC]
    o_i = tl.full([BQ, BC], -1, dtype=tl.int32) #[BQ, BC]
    m_i = (tl.arange(0, BC) < BC//2)[None,:] #[1, BC]

    IC = (i_t + offs_b) // BS #[BQ]
    MAX_IC = (i_t + BQ - 1) // BS
    
    for i_c in range(0, MAX_IC + 1, BC):
        o_c = i_c + tl.arange(0, BC) # [BC]

        p_k = tl.make_block_ptr(k + (boc * H + i_h) * D, (D, TC), (1, H*D), (0, i_c), (BK, BC), (0, 1))
        # [D, BC]
        b_k = tl.load(p_k, boundary_check=(0, 1))
        # [BQ * G, BC]
        b_s = tl.dot(b_q, b_k)
        b_s = tl.reshape(b_s, [BQ, G, BC])
        
        b_s = tl.where((o_c[None,:] < IC[:, None])[:,None,:], b_s, NEG_INF)
        # [BQ, G, BC]
        # the 1st and the last 2 blocks are always selected
        
        
        b_p = tl.where(((o_c[None,:] == 0) 
                    | (o_c[None,:] == IC[:, None] - 1) 
                    | (o_c[None,:] == IC[:, None])
                    )[:,None,:],  
                INF, b_s)
        ...
        # [BQ, BC]
        b_i, b_ip = tl.sum(b_p, 1), b_i
        # blocks with index < 0 will be skipped
        o_i, o_ip = tl.where((o_c[None,:] <= IC[:,None]), o_c[None,:], -1), o_i

        n_dims: tl.constexpr = tl.standard._log2(b_i.shape[1])
    ...
    b_top = tl.sum(m_top[None,:, None] * tl.reshape(o_i, [BQ, BC//S, S]), 1)

    base = (bos + i_t) * H * S + i_h * S
    offs = base + tl.arange(0, BQ)[:,None] * H * S + tl.arange(0, S)
    p_b = block_indices + offs
...
def parallel_nsa_fwd_kernel(
    ...
    # [G, BK]
    b_q = tl.load(p_q, boundary_check=(0, 1), cache_modifier=".cv")
    b_q = (b_q * scale).to(b_q.dtype)
...
def parallel_nsa_topk(
    ...
    q_chunk_indices = prepare_chunk_indices(cu_seqlens, BQ) if cu_seqlens is not None else None
    NT = triton.cdiv(T, BQ) if cu_seqlens is None else len(q_chunk_indices)
    grid = (NT, B * H)
    # the 1st and the last 2 blocks are always selected
...
window_offset: int = 0
    ...
    block_indices = nsa_topk(
        q, k, v, cu_seqlens, block_size, block_counts, window_offset
    )
...
window_offset: int = 0
    ...
    with torch.no_grad():
        ...
        q_cmp = rearrange(q, 'b t (h g) d -> b t h g d', g=G).mean(-2)
