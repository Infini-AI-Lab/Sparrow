...
def compute_reward_metrics(reward_tensor, batch) -> dict[str, Any]:
    """
    Computes only the critic reward statistics (mean, max, min) for a batch.
    """
    # sequence_reward = batch.batch["token_level_rewards"].sum(-1) 
    ...
    if "response_mask" in batch.batch:
        ...
        response_length = _compute_response_info(batch)["response_length"]

    non_aborted_sequence_reward = sequence_reward[response_length != 0]
    ...
    return {
        "critic/rewards_unfiltered/mean": reward_mean,
        "critic/rewards_unfiltered/max": reward_max,
        "critic/rewards_unfiltered/min": reward_min,
    } 
