...
__all__ = ["DataProto", "union_tensor_dict", "deepcopy_dataproto"]
...
def deepcopy_dataproto(data: "DataProto") -> "DataProto":
    """
    Create a deep copy of a DataProto.

    TensorDict provides .clone() to duplicate underlying storage; we also clone
    non-tensor fields and meta_info separately to avoid shared references.
    """
    batch_copy = data.batch.clone() if data.batch is not None else None
    non_tensor_batch_copy = (
        {key: np.copy(val) for key, val in data.non_tensor_batch.items()} if data.non_tensor_batch is not None else {}
    )
    meta_info_copy = copy.deepcopy(data.meta_info) if data.meta_info is not None else None

    return type(data)(batch=batch_copy, non_tensor_batch=non_tensor_batch_copy, meta_info=meta_info_copy)
