...
def _torch_save_with_legacy_fallback(state_dict, path: str, rank: int, payload_name: str) -> None:
    """Persist a state dict and fall back to legacy serialization for large zip-writer failures."""
    try:
        torch.save(state_dict, path)
        return
    except RuntimeError as exc:
        err_msg = str(exc)
        zip_write_failed = "PytorchStreamWriter failed" in err_msg or "unexpected pos" in err_msg
        if not zip_write_failed:
            raise
        ...
        log_with_rank(
            (
                f"Retrying {payload_name} save with legacy torch serialization after zip writer failure: "
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None, exception_step: Optional[int] = None): 
        """
        ...
        if exception_step is not None and isinstance(exception_step, str): 
            if ',' not in exception_step: 
                try: 
                    exception_step = [int(exception_step)] 
                except ValueError: 
                    raise ValueError("exception_step must be an integer or a string of integers separated by commas") 
            else: 
                try: 
                    exception_step = [int(step) for step in exception_step.split(',')] 
                except ValueError: 
                    raise ValueError("exception_step must be a string of integers separated by commas") 
        
        if exception_step is not None: 
            keyword_for_exception = ["global_step_" + str(x) for x in exception_step] 
            print("*** keyword_for_exception {}".format(keyword_for_exception), flush = True) 
        else: 
            keyword_for_exception = None 
        ...
        if (
            ...
            remove_lists = self.previous_saved_paths[:keep_start] 
            keep_lists = [] 
            print("*** remove_lists {}".format(remove_lists), flush = True) 
            if keyword_for_exception is not None: 
                newlists = [] 
                for path in remove_lists: 
                    print("*** path {}".format(path), flush = True) 
                    found = False 
                    for keyword in keyword_for_exception: 
                        if keyword in path: 
                            keep_lists.append(path) 
                            found = True 
                            break 
                    if not found: 
                        newlists.append(path) 
                remove_lists = newlists 
                print("*** keep_lists {}".format(keep_lists), flush = True) 
                print("*** newlists {}".format(newlists), flush = True) 
            # self.remove_previous_save_local_path(self.previous_saved_paths[:keep_start]) 
            self.remove_previous_save_local_path(remove_lists) 
            self.previous_saved_paths = self.previous_saved_paths[keep_start:] 
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    ...
                    _torch_save_with_legacy_fallback(
                        model_state_dict, model_path, rank=self.rank, payload_name="model shard"
                    )
                ...
                if self.should_save_optimizer:
                    ...
                    _torch_save_with_legacy_fallback(
                        optimizer_state_dict, optim_path, rank=self.rank, payload_name="optimizer shard"
                    )
                ...
                if self.should_save_extra:
                    ...
                    _torch_save_with_legacy_fallback(
                        extra_state_dict, extra_path, rank=self.rank, payload_name="extra state"
                    )
        ...
        if "persistent" not in local_path:
