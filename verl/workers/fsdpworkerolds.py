...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode(self, should_update_model_weights=True, use_alternative = False): 
        """Context switch hybridengine to rollout mode.""" 
        if not use_alternative: 
            if self.rollout_alternative is not None: 
                print("###### Detecting rollout mode but rollout_alternative is not None, shutting down rollout_alternative ######", flush = True) 
                await self.rollout.release() 
                dist.barrier() 
                await self.rollout_alternative.shutdown() 
                self.rollout_alternative = None 
                # free memory 
                aggressive_empty_cache(force_sync=True) 
                del self.rollout_alternative 
                self.rollout_alternative = None 
            if self.rollout is None: 
                print("###### Detecting rollout mode but rollout is not initialized, initializing rollout ######", flush = True) 
                self._build_rollout(use_alternative = False, skip_trainermode = True) 
        else: 
            if self.rollout is not None: 
                print("###### Detecting rollout mode but rollout is not None, shutting down rollout ######", flush = True) 
                dist.barrier() 
                await self.rollout.shutdown() 
                self.rollout = None 
                # free memory 
                aggressive_empty_cache(force_sync=True) 
                del self.rollout 
                self.rollout = None 
            if self.rollout_alternative is None: 
                print("###### Detecting rollout mode but rollout_alternative is not initialized, initializing rollout_alternative ######", flush = True) 
                self._build_rollout(use_alternative = True, skip_trainermode = True) 
        
        rollout_engine = self.rollout_alternative if use_alternative else self.rollout 
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if self._is_actor:  # For rollout only, we do not switch context.
            loop = asyncio.get_event_loop()
            loop.run_until_complete(self.rollout_mode(should_update_rollout, use_alternative=use_alternative)) 
        ...
        with simple_timer("generate_sequences", timing_generate): 
            if not use_alternative: 
                ...
                print("######### use_alternative generating sequences #########") 
                output = self.rollout_alternative.generate_sequences(prompts=prompts) 
