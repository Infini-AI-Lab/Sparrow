...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False, use_sparse = False, skip_trainermode = False): 
        from torch.distributed.device_mesh import init_device_mesh
        ...
        rollout_device_mesh_sparse = init_device_mesh(
            device_name, mesh_shape=(dp, infer_tp), mesh_dim_names=["dp", "infer_tp"]
        ) 
        ...
        if rollout_name == "hf":
            ...
            if not use_sparse: 
                ...
                try:
                    self._register_dispatch_collect_info(
                        "rollout_sparse", dp_rank=rollout_device_mesh_sparse["dp"].get_local_rank(), is_collect=is_collect 
                    ) 
                except ValueError:
                    # already registered; reuse existing mapping
                    pass
        ...
        if not use_sparse: 
            ...
            self.rollout_sparse = None 
        else: 
            assert self.rollout is None 
            self.rollout = None 
            sparsespecificconfig = copy.deepcopy(rollout_config) 
            dictsglang = {} 
            ...
            sparsespecificconfig.engine_kwargs["sglang"] = dictsglang 
            self.rollout_sparse = get_rollout_class(rollout_config.name, rollout_config.mode)(
                config = sparsespecificconfig, model_config=model_config, device_mesh=rollout_device_mesh_sparse 
            ) 
            print("*************** rollout_sparse initialized ***************") 
    ...
    async def rollout_mode(self, should_update_model_weights=True, use_sparse=False): 
        """Context switch hybridengine to rollout mode.""" 
        if not use_sparse: 
            if self.rollout_sparse is not None: 
                print("###### Detecting rollout mode but rollout_sparse is not None, shutting down rollout_sparse ######", flush = True) 
                await self.rollout_sparse.shutdown() 
                self.rollout_sparse = None 
                # free memory 
                aggressive_empty_cache(force_sync=True) 
                del self.rollout_sparse 
                self.rollout_sparse = None 
            if self.rollout is None: 
                print("###### Detecting rollout mode but rollout is not initialized, initializing rollout ######", flush = True) 
                self._build_rollout(use_sparse = False, skip_trainermode = True) 
        else: 
            if self.rollout is not None: 
                ...
                del self.rollout 
                self.rollout = None 
            if self.rollout_sparse is None: 
                print("###### Detecting rollout mode but rollout_sparse is not initialized, initializing rollout_sparse ######", flush = True) 
                self._build_rollout(use_sparse = True, skip_trainermode = True) 
        
        rollout_engine = self.rollout_sparse if use_sparse else self.rollout 
    ...
    async def trainer_mode(self):
        ...
        if self.config.rollout.free_cache_engine:
            ...
            if self.config.actor.use_sparse_training and self.rollout_sparse is not None: 
                await self.rollout_sparse.release() 
                log_gpu_memory_usage("After rollout_sparse offload", logger=logger) 
                print("###### After rollout_sparse release ######", flush = True) 
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        use_sparse = prompts.meta_info.get("use_sparse", False) 
        ...
        if self._is_actor:  # For rollout only, we do not switch context.
            loop = asyncio.get_event_loop()
            loop.run_until_complete(self.rollout_mode(should_update_rollout, use_sparse=use_sparse)) 
        ...
        with simple_timer("generate_sequences", timing_generate): 
            if not use_sparse: 
                ...
                output = self.rollout_sparse.generate_sequences(prompts=prompts) 
