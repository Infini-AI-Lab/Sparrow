...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.config.actor.use_sparse_training:
    ...
    def compute_get_raw_acceptance_rate(self, data: DataProto): 
        import math 
        model_inputs = {**data.batch, **data.non_tensor_batch} 

        if not self.config.actor.use_double_min_sample:
            raise ValueError(
                "compute_get_raw_acceptance_rate requires "
                "`actor.use_double_min_sample=True` because it consumes rollout/actor top-k tensors."
            )
        
        rollout_log_probs_topk_idx = (
            model_inputs["rollout_log_probs_topk_idx"] if self.config.actor.use_double_min_sample else None
        )
        rollout_log_probs_topk_logprob = (
            model_inputs["rollout_log_probs_topk_logprob"] if self.config.actor.use_double_min_sample else None
        ) 
        
        # Twin-engine workers typically emit generic old_log_probs fields, while
        # sparse/dense variants use olddense_* names. Accept either layout here.
        dense_oldoutput = self.compute_log_prob(data) 
        old_log_prob = dense_oldoutput.batch["old_log_probs"] 
        old_topk_idx = dense_oldoutput.batch["old_topk_idx"] if self.config.actor.use_double_min_sample else None
        old_topk_logp = dense_oldoutput.batch["old_topk_logp"] if self.config.actor.use_double_min_sample else None 

        missing_inputs = []
        if rollout_log_probs_topk_idx is None:
            missing_inputs.append("rollout_log_probs_topk_idx")
        if rollout_log_probs_topk_logprob is None:
            missing_inputs.append("rollout_log_probs_topk_logprob")
        if old_topk_idx is None:
            missing_inputs.append("old_topk_idx")
        if old_topk_logp is None:
            missing_inputs.append("old_topk_logp")
        if missing_inputs:
            raise ValueError(
                "compute_get_raw_acceptance_rate is missing top-k tensors: "
                f"{', '.join(missing_inputs)}. "
                "This debug path requires rollout top-k capture and actor top-k recomputation. "
                "Check that `actor.use_double_min_sample=True`, "
                "`rollout.calculate_log_probs=True`, and the actor log-prob path is actually returning top-k tensors."
            )
        ...
        overlap_mass = verl_F.sum_min_from_topk(
            rollout_log_probs_topk_idx,
            rollout_log_probs_topk_logprob,
            old_topk_idx,
            old_topk_logp,
            double_min_threshold,
        ) 
        acceptance_rate_raw = overlap_mass 
        acceptance_rate_raw.mul_(response_mask.to(acceptance_rate_raw.dtype))
        return DataProto.from_dict(
            tensors={
                "acceptance_rate_raw": acceptance_rate_raw,
            }
        ).to("cpu") 

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor")) 
    @DistProfiler.annotate(color="red", role="actor_update") 
    def compute_acceptance_rate_sequence_dense(self, data: DataProto): 
        import math 
        model_inputs = {**data.batch, **data.non_tensor_batch} 
        ...
        mask, accepted_cnt, total_cnt = verl_F.bernoulli_mask_min_ratio_from_logp(
            rollout_log_probs, old_log_prob, double_min_threshold
        )
        mask_kept = (response_mask & mask)

        # Per-token diagnostic weights (unchanged)
        with torch.no_grad():
            log_r = (old_log_prob - rollout_log_probs - math.log(double_min_threshold)).clamp(-20.0, 20.0)
            log_s = torch.relu(log_r)
            s = torch.exp(log_s)
            s_mean_kept = verl_F.masked_mean(s, mask_kept).clamp_min(1e-12)
            log_w_metric = log_s - torch.log(s_mean_kept)
            w_metric = torch.exp(log_w_metric)

        ppo_kl_double_min = verl_F.masked_mean(w_metric.detach() * log_w_metric.detach(), mask_kept)

        delta_old_vs_inf = (rollout_log_probs - old_log_prob).detach()
        ppo_kl_vllm = verl_F.masked_mean(delta_old_vs_inf, response_mask)

        seq_len = response_mask.shape[-1] 
        print("*** seq_len {} ***".format(seq_len), flush = True) 
        bucket_size = 2048
        bucket_starts = torch.arange(0, seq_len, bucket_size, device=response_mask.device)
        bucket_ends = (bucket_starts + bucket_size).clamp_max(seq_len)
        print("*** bucket_starts {} ***".format(bucket_starts), flush = True) 
        print("*** bucket_ends {} ***".format(bucket_ends), flush = True) 

        acceptance_rate = []
        mask_kept_float = mask_kept.to(torch.float32)
        response_mask_float = response_mask.to(torch.float32)
        for start, end in zip(bucket_starts.tolist(), bucket_ends.tolist()):
            accepted_in_bucket = mask_kept_float[:, start:end].sum()
            total_in_bucket = response_mask_float[:, start:end].sum()
            acceptance_rate.append(accepted_in_bucket / (total_in_bucket + 1e-6))
        acceptance_rate = torch.stack(acceptance_rate)
        return DataProto.from_dict(
            tensors={"acceptance_rate": acceptance_rate}
        ).to("cpu") 
    
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor")) 
    @DistProfiler.annotate(color="red", role="actor_update") 
    def compute_acceptance_rate(self, data: DataProto): 
        import math 
        model_inputs = {**data.batch, **data.non_tensor_batch} 
        ...
        acceptance_rate = ((mask_kept).sum().item()) / (response_mask.sum().item() + 1e-6)
        return DataProto.from_dict(
            tensors={"acceptance_rate": torch.tensor([acceptance_rate], dtype=torch.float32, device=response_mask.device)}
        ).to("cpu")
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, topk_idx, topk_logp = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            if topk_idx is None:
                tensors_dict = {"old_log_probs": output, "entropys": entropys}
            else:
                tensors_dict = {"old_log_probs": output, "entropys": entropys, "old_topk_idx": topk_idx, "old_topk_logp": topk_logp}
