...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.config.get("use_sparse_training", False): 
                    # print(f"***************** in dp actor use_sparse_training={self.config.use_sparse_training} block size {self.config.bsa_kwargs_nsa_block_size} block counts {self.config.bsa_kwargs_nsa_block_counts} window offset {self.config.bsa_kwargs_window_offset} *****************") 
                    extra_args["nsa_block_size"] = self.config.bsa_kwargs_nsa_block_size 
                    extra_args["nsa_block_counts"] = self.config.bsa_kwargs_nsa_block_counts 
                    extra_args["window_offset"] = self.config.bsa_kwargs_window_offset 

                print(f"***** input_ids_rmpad {input_ids_rmpad.shape} *****", flush = True) 
                ...
                if self.use_fused_kernels:
                    ...
                    if not self.actor_optimizer is None:
                ...
                if self.config.use_sparse_training: 
                    extra_args["nsa_block_size"] = self.config.bsa_kwargs_nsa_block_size 
                    extra_args["nsa_block_counts"] = self.config.bsa_kwargs_nsa_block_counts 
                    extra_args["window_offset"] = self.config.bsa_kwargs_window_offset 

                print("input_ids shape", input_ids.shape)
                print("attention_mask shape", attention_mask.shape if attention_mask is not None else "no found") 
                print("position_ids shape", position_ids.shape if position_ids is not None else "no found") 
                output = self.actor_module(
                    input_ids=input_ids,
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                entropy, log_probs, topk_idx, topk_logp = self._forward_micro_batch(
    ...
    def update_policy(self, data: DataProto):
        ...
        if self.config.use_double_min_sample:
            ...
            select_keys.append("old_topk_idx")
            select_keys.append("old_topk_logp")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    old_topk_idx = model_inputs["old_topk_idx"] if self.config.use_double_min_sample else None
                    old_topk_logp = model_inputs["old_topk_logp"] if self.config.use_double_min_sample else None
                    ...
                    entropy, log_prob, topk_idx, topk_logp = self._forward_micro_batch(
                        model_inputs, temperature=temperature, calculate_entropy=calculate_entropy
                    )
                    ...
                    if on_policy:
                        ...
                        if self.config.use_double_min_sample and not self.config.double_min_apply_trust_region:
                            old_topk_idx = topk_idx.detach()
                            old_topk_logp = topk_logp.detach()
                    else:
                        if self.config.use_double_min_sample:
                            if self.config.double_min_use_latest_logits and not self.config.double_min_apply_trust_region:
                                old_log_prob = log_prob.detach()
                                old_topk_idx = topk_idx.detach()
                                old_topk_logp = topk_logp.detach()
                            else:
                                old_log_prob = model_inputs["old_log_probs"]
                                old_topk_idx = model_inputs["old_topk_idx"]
                                old_topk_logp = model_inputs["old_topk_logp"]     
                        else:
                    ...
                    if self.config.use_double_min_sample:
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, acceptance_rate, ppo_kl_vllm, ppo_kl_double_min = policy_loss_fn(
                            ...
                            old_topk_idx=old_topk_idx,
                            old_topk_logp=old_topk_logp,
                        ) 
                    else:
                    ...
                    if self.config.use_double_min_sample:
                        double_min_sample_metrics_dict = {
                            "actor/sampler_acceptance_rate": acceptance_rate,
                            "actor/ppo_kl_vllm": ppo_kl_vllm.detach().item(),
                            "actor/ppo_kl_double_min": ppo_kl_double_min.detach().item()
                        }
