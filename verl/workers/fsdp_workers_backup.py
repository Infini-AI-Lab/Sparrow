...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self._last_rollout_sync_step = 0
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        self.layered_summon = self.config.rollout.get("layered_summon", False) if self.config.rollout.rollout_model_update_every_nsteps == 1 else True
    ...
    async def rollout_mode(self, should_update_model_weights=True):
        ...
        if hasattr(peft_model, "peft_config"):  # LoRA
            ...
            params = collect_lora_params(
                ...
                layered_summon=self.config.rollout.get("layered_summon", False) if self.config.rollout.rollout_model_update_every_nsteps == 1 else True,
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if self.config.rollout.rollout_model_update_every_nsteps > 1 and global_step is not None:
            if self._last_rollout_sync_step is None:
                ...
                if delta < 0:
                    ...
                    should_update_rollout = delta >= self.config.rollout.rollout_model_update_every_nsteps
        elif self.config.rollout.rollout_model_update_every_nsteps > 1 and global_step is None:
            should_update_rollout = True
            
        if should_update_rollout:
            self._last_rollout_sync_step=global_step

        timing_generate = {}
        if self._is_actor:  # For rollout only, we do not switch context.
            loop = asyncio.get_event_loop()
            loop.run_until_complete(self.rollout_mode(should_update_rollout))
