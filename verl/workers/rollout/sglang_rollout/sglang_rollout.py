...
from verl.utils.torch_functional import get_response_mask, pad_sequence_to_length, pad_3d_along_dim1_to_length
...
class AsyncEngine(sglang.srt.entrypoints.engine.Engine):
    ...
    async def shutdown(self):
        """Async wrapper for Engine.shutdown()."""
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, super().shutdown)
...
def _extract_rollout_topk_from_response(
    resp,
    logprobs_to_keep: int,
    use_fast_toplogprobs_path: bool,
):
    # The fast path skips OpenAI-format top-k materialization in SGLang and
    # consumes the raw per-position idx/logprob lists directly. Both fast and
    # legacy paths are normalized into the same nested list shape here so the
    # downstream tensorization logic stays identical.
    meta_info = resp.get("meta_info", {})
    sampled_output_logprobs = meta_info.get("output_token_logprobs", [])

    if use_fast_toplogprobs_path:
        current_idx = meta_info.get("output_top_logprobs_raw_idx", [])
        current_lp = meta_info.get("output_top_logprobs_raw_logprob", [])
    else:
        output_top_logprobs = meta_info.get("output_top_logprobs", [])
        current_idx = []
        current_lp = []
        for topk_candidates in output_top_logprobs:
            if not isinstance(topk_candidates, (list, tuple)):
                current_idx.append([])
                current_lp.append([])
                continue
            try:
                ...
                sorted_candidates = topk_candidates[:logprobs_to_keep]
    ...
    if len(current_idx) != len(current_lp):
        raise ValueError(
            "Mismatched top-k idx/logprob lengths "
            f"for request {meta_info.get('id')}: {len(current_idx)} vs {len(current_lp)}"
        )
    if len(current_idx) != len(sampled_output_logprobs):
        raise ValueError(
            "Top-k position count does not align with sampled output_token_logprobs "
            f"for request {meta_info.get('id')}: {len(current_idx)} vs {len(sampled_output_logprobs)}"
        )
    for pos, (idx_candidates, lp_candidates) in enumerate(zip(current_idx, current_lp)):
        if len(idx_candidates) != len(lp_candidates):
            raise ValueError(
                "Mismatched top-k candidate counts "
                f"for request {meta_info.get('id')} position {pos}: "
                f"{len(idx_candidates)} vs {len(lp_candidates)}"
            )

    return current_idx, current_lp
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if effective_first:
            ...
            args = {
                ...
                "enable_fast_toplogprobs_path": self.config.enable_fast_toplogprobs_path,
                **engine_kwargs, 
            } 
    ...
    @GPUMemoryLogger(role="sglang rollout", logger=logger)
    @torch.no_grad()
    def _batch_level_per_rank_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # dense_use = prompts.meta_info["dense_use"] 
        LOGPROBS_TO_KEEP = self.config.log_probs_to_keep
        use_fast_toplogprobs_path = self.config.enable_fast_toplogprobs_path
        ...
        if not do_sample:
            ...
            # training sampling: request top-k logprobs; pass via async_generate arg, not sampling_params
            pass
        ...
        if self._tp_rank == 0:
            ...
            output = loop.run_until_complete(
                self._engine.async_generate(
                    ...
                    return_logprob=True, 
                    top_logprobs_num=(LOGPROBS_TO_KEEP if (self.config.calculate_log_probs and not is_validate) else 0),
                    input_ids=idx_list,
            ...
            output = None 
        import datetime 
        print("*** datetime {}, after generate sequences ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
        dist.barrier() 

        if self._tp_size > 1:
            # Most naive implementation, can extract tensor and send via gloo if too slow 
            print("*** datetime {}, after barrier ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
        ...
        print("*** datetime {}, after broadcast ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
        out = _post_process_outputs(self.processing_class, output)

        # Extract per-position top-k candidate token ids and logprobs if requested
        rollout_log_probs_topk_idx = None
        rollout_log_probs_topk_logprob = None
        if self.config.calculate_log_probs and not is_validate and LOGPROBS_TO_KEEP and LOGPROBS_TO_KEEP > 0:
            topk_idx_list = []
            topk_logprob_list = []
            for resp in output:
                current_idx, current_lp = _extract_rollout_topk_from_response(
                    resp=resp,
                    logprobs_to_keep=LOGPROBS_TO_KEEP,
                    use_fast_toplogprobs_path=use_fast_toplogprobs_path,
                )
                topk_idx_list.append(current_idx)
                topk_logprob_list.append(current_lp)

            # Pad along sequence length (dim1) to response_length
            rollout_log_probs_topk_idx = pad_3d_along_dim1_to_length(
                topk_idx_list,
                pad_to=self.config.response_length,
                device=idx.device,
            )
            rollout_log_probs_topk_logprob = pad_3d_along_dim1_to_length(
                topk_logprob_list,
                pad_to=self.config.response_length,
                device=idx.device,
                dtype=torch.float32,
        ...
        if self.config.calculate_log_probs:
            ...
            if not is_validate and rollout_log_probs_topk_idx is not None:
                batch["rollout_log_probs_topk_idx"] = rollout_log_probs_topk_idx
                batch["rollout_log_probs_topk_logprob"] = rollout_log_probs_topk_logprob
    ...
    async def _handle_engine_generate(
        ...
        # enable returning logprobs when top_logprobs is requested
        return_logprob = bool(kwargs.get("top_logprobs", 0))
    ...
    async def shutdown(self):
        ...
        # Best-effort: abort in-flight requests to unblock cache flush
        try:
            abort_fn = getattr(self._engine, "abort_request", None)
            if abort_fn is not None:
                if asyncio.iscoroutinefunction(abort_fn):
                    await asyncio.wait_for(abort_fn(abort_all=True), timeout=5.0)
                else:
                    await loop.run_in_executor(None, lambda: abort_fn(abort_all=True))
        except Exception as e:
            logger.warning(f"shutdown(): abort_request failed: {e}")

        # Flush cache and optionally release memory on TP master only
        try:
            if self.device_mesh["infer_tp"].get_local_rank() == 0:
                flush_fn = getattr(self._engine, "flush_cache", None)
                if flush_fn is not None:
                    try:
                        if asyncio.iscoroutinefunction(flush_fn):
                            await asyncio.wait_for(flush_fn(), timeout=10.0)
                        else:
                            await loop.run_in_executor(None, flush_fn)
                    except Exception as e:
                        logger.warning(f"shutdown(): flush_cache failed: {e}")

                if self.config.free_cache_engine:
                    release_fn = getattr(self._engine, "release_memory_occupation", None)
                    if release_fn is not None:
                        try:
                            if asyncio.iscoroutinefunction(release_fn):
                                await asyncio.wait_for(
                                    release_fn(tags=["kv_cache", "weights"]), timeout=10.0
                                )
                            else:
                                await loop.run_in_executor(
                                    None, lambda: release_fn(tags=["kv_cache", "weights"]) 
                                )
                        except Exception as e:
                            logger.warning(f"shutdown(): release_memory_occupation failed: {e}")
        except Exception as e:
            logger.warning(f"shutdown(): pre-shutdown cleanup failed: {e}")

        # Final engine shutdown (supports both async/sync implementations)
        try:
            engine_shutdown = getattr(self._engine, "shutdown", None)
            if engine_shutdown is None:
                return
            if asyncio.iscoroutinefunction(engine_shutdown):
                await asyncio.wait_for(engine_shutdown(), timeout=15.0)
            else:
                await loop.run_in_executor(None, engine_shutdown)
        except Exception as e:
            logger.warning(f"shutdown(): engine shutdown failed: {e}")
