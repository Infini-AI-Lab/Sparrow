...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # debugging 
        import datetime 
        print("*** datetime {}, after wait generation ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
        ...
        # debugging 
        output_size = len(output) if hasattr(output, "__len__") else "unknown"
        print(f"*** broadcast output size {output_size} ***", flush=True)
        ...
        if self.config.calculate_log_probs and not is_validate and LOGPROBS_TO_KEEP and LOGPROBS_TO_KEEP > 0:
            ...
            topk_logprob_list = []
            for resp in output:
                # SGLang returns top-k as meta_info["output_top_logprobs"]:
                # a list (per position) of top candidates, each candidate as [logprob, token_id, token_text]
                output_top_logprobs = resp.get("meta_info", {}).get("output_top_logprobs", [])
                current_idx = []
                current_lp = []
                for topk_candidates in output_top_logprobs:
                    # topk_candidates: list of [logprob, token_id, token_text]
                    if not isinstance(topk_candidates, (list, tuple)):
                        current_idx.append([])
                        current_lp.append([])
                        continue
                    # Ensure sorted by logprob desc and truncated to LOGPROBS_TO_KEEP
                    try:
                        sorted_candidates = sorted(
                            topk_candidates,
                            key=lambda x: float(x[0]) if isinstance(x, (list, tuple)) and len(x) > 0 else -1e9,
                            reverse=True,
                        )[:LOGPROBS_TO_KEEP]
                    except Exception:
                        sorted_candidates = topk_candidates[:LOGPROBS_TO_KEEP]

                    current_idx.append([int(c[1]) for c in sorted_candidates if isinstance(c, (list, tuple)) and len(c) >= 2])
                    current_lp.append([float(c[0]) for c in sorted_candidates if isinstance(c, (list, tuple)) and len(c) >= 1])
        ...
        # debugging 
        print("*** datetime {}, after pad 3d along dim1 to length ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
        ...
        # debugging 
        print("*** datetime {}, after get response attention mask ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
        ...
        # debugging 
        print("*** datetime {}, after flush cache ***".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), flush = True) 
