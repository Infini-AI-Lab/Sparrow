...
class TokenizerManager:
    ...
    def convert_logprob_style(
        ...
        fast_toplogprobs_path = self.server_args.enable_fast_toplogprobs_path
        ...
        if top_logprobs_num > 0:
            ...
            if fast_toplogprobs_path:
                # For non-streaming rollout we only need the final accumulated raw
                # top-k payload. Deferring publication avoids repeated full-history
                # copies or OpenAI-format materialization on every incremental batch.
                if recv_obj.finished_reasons[recv_obj_index] is not None:
                    meta_info["output_top_logprobs_raw_idx"] = (
                        state.output_top_logprobs_idx
                    )
                    meta_info["output_top_logprobs_raw_logprob"] = (
